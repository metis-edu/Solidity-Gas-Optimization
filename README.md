...
Consider using bitwise operators for arithmetic operations where applicable, as they often consume less gas. However, note that bit shift (<< and >>) operators are not arithmetic operators and do not revert on overflow.
...
The `public` visibility modifier is equivalent to `external` plus `internal`. `Public` can be called both externally and internally, whereas `external` can only be called from outside the contract. Hence, prefer `external` to save gas, unless `internal` calls are needed.
...
Variables have default values (0, false, 0x0, etc., depending on the data type) if not explicitly initialized. Explicit initialization with default values wastes gas.
...
Appending error reason strings with `require` statements is a best practice, but these strings consume space in the deployed bytecode. Each reason string needs at least 32 bytes. Keep your strings within 32 bytes to minimize cost.
...
Eliminate unnecessary checks to save gas.
...
Using nested `if` statements is cheaper than using multiple `&&` combinations. It also leads to easier-to-read code and better coverage reports.
...
Using multiple `require` statements is cheaper than using multiple `&&` combinations. It also leads to easier-to-read code and better coverage reports.
...
Set appropriate function visibility, as `internal` functions are cheaper to call than `public` functions. There is no need to mark a function as `public` if it is only meant to be called internally.
